...
- **GitHub 토큰/비밀번호**: 코드나 채팅에 넣지 마세요. 이 프로젝트는 Git 인증을 **macOS 키체인(Keychain)**에 저장하도록 설정돼 있습니다. 터미널에서 `git push` 할 때 비밀번호를 묻면 토큰을 한 번 입력해 두면 이후엔 키체인에 저장되어 자동으로 사용됩니다.
- **`.env`**: API 키·비밀은 `.env`에만 두고, **절대 커밋하지 마세요.** (`.env`는 이미 `.gitignore`에 포함돼 있습니다.)
